            ...
            <form action="{{ route('blog.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mt-4 input-style-1">
                    ...
                    <textarea id="description" name="description" rows="5" 
                        class="form-control @error('description') is-invalid @enderror" placeholder="Enter Blog Description">{{ old('description') }}</textarea>
                    @error('description')
                        <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                    @enderror
                </div>
                ...
                <div class="mt-4 input-style-1">
                    ...
                    <input type="text" id="blog_creator" name="blog_creator" 
                        ...
                        value="{{ old('blog_creator') }}" placeholder="Enter Blog Creator Name" />
                    @error('blog_creator')
                        <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                    @enderror
                </div>
